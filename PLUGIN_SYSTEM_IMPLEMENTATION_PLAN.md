...
action = PluginAction(
    ...
    url = ref.action.url,
    hostActionId = ref.action.hostActionId   // ← 新增
)
```
...
**初始化**：在 `TermuxApplication.onCreate()` 或主 Activity 启动时调用 `HostActionRegistry.registerDefaults(context)`。
...
object ComposeRenderer {
    ...
    private fun RenderLazyColumn(
        ...
        LazyColumn {
            items(items) { item ->
                itemTemplate?.let { template ->
                    val resolved = resolveTemplate(template, item)
    ...
    private fun parseShellOutput(raw: String): List<Map<String, Any?>> {
        ...
        // fallback: 按空白分割，每行一个 {serial, status}
...
资源卡片点击处（`ResourcesScreen` 等）把硬编码的 shell/url 判断改成统一路由：
...
```
阶段 1 (ActionExecutor + HostActionRegistry)    ← 零依赖，先搞
    │
    ├─→ 阶段 3 (JS Bridge 扩展)                 ← 依赖 1，可并行
    │
    ├─→ 阶段 2 (Compose DSL Renderer)            ← 依赖 1（ActionExecutor 处理事件）
    │
    └─→ 阶段 4 (PluginCenterActivity 适配)      ← 依赖 1 + 2
```
