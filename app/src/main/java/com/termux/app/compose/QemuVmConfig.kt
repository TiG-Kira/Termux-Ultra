...
data class QemuVmConfig(
    ...
    private fun generateNativeScript(): String {
        ...
        if (effectiveHasSound) {
            ...
            when (effectiveAudioMode) {
                ...
                AudioMode.PA_FOLLOW_SCREEN, AudioMode.PA_PERSIST -> {
                    ...
                    sb.append("        # 第二步：卸载可能存在的 ALSA sink，避免 QEMU 音频直接输出到物理扬声器\n")
                    sb.append("        pactl unload-module module-alsa-sink 2>/dev/null || true\n")
                    sb.append("        pactl unload-module module-alsa-card 2>/dev/null || true\n")
                    sb.append("        sleep 0.3\n")
                    sb.append("        # 第三步：加载必要的模块\n")
                    ...
                    sb.append("        sleep 0.3\n")
                    sb.append("        # 将 null-sink 设为默认 sink\n")
                    sb.append("        pactl set-default-sink auto_null 2>/dev/null || true\n")
                    sb.append("        pactl set-default-source auto_null.monitor 2>/dev/null || true\n")
                    sb.append("        sleep 0.3\n")
                    ...
                    sb.append("            ACTUAL_DEFAULT_SINK=\$(pactl get-default-sink 2>/dev/null || echo unknown)\n")
                    sb.append("            echo \"PA 就绪: default-sink=\$ACTUAL_DEFAULT_SINK (预期 auto_null)\"\n")
                    sb.append("            echo 'PulseAudio TCP 4713 + 4714 双端口已就绪'\n")
                    sb.append("        else\n")
                    sb.append("            echo 'WARN: PulseAudio 端口未全部就绪，PA 音频不可用'\n")
                    sb.append("            HAS_PULSEAUDIO=0\n")
                    sb.append("        fi\n")
                    sb.append("    fi\n")
                    sb.append("else\n")
        ...
        sb.append("    # sink=auto_null 确保输出到 null-sink（monitor 供 Android 播放器读取）\n")
        sb.append("    EXTRA_AUDIO_ARGS=(\n")
        sb.append("        -audiodev pa,id=pa_audio,server=tcp:127.0.0.1:4713,sink=auto_null\n")
        sb.append("        -device ich9-intel-hda\n")
    ...
    private fun generateContainerScript(): String {
        ...
        // 2.5 在 Termux 原生层启动音频后端
        // 核心改造：PulseAudio daemon 不再在容器内启动（proot 下 shm/root/machine-id 等问题
        // 导致三级 fallback 仍不稳），改为在 Termux 原生层启动 PA daemon，容器内 QEMU 通过
        // 共享的网络栈直连 127.0.0.1:4713。proot 不做网络 namespace 隔离，网络栈完全共享。
        //
        // VNC RFB audio driver 作为可选增强：先检测容器内 QEMU 是否支持 vnc audio driver，
        // 若支持则优先使用（音频随 VNC 连接走），否则自动降级到原生层 PA。
        if (effectiveHasSound) {
            ...
            // ── 先尝试 VNC RFB audio driver（仅对 VNC_RFB 模式有意义）──
            when (effectiveAudioMode) {
                AudioMode.VNC_RFB -> {
                    sb.append("echo \"音频模式: VNC RFB 扩展（优先尝试，失败自动回退 PulseAudio）\"\n")
                    ...
                    sb.append("    HAS_VNC_AUDIO=1\n")
                    sb.append("else\n")
                    sb.append("    echo \"容器内 QEMU 未检测到 vnc audio driver，回退到 PulseAudio\"\n")
            ...
            // ── 无论用户选的是 PA 还是 VNC_RFB（回退路径），都在 Termux 原生层启动 PA daemon ──
            sb.append("# 在 Termux 原生层安装并启动 PulseAudio daemon\n")
            sb.append("if ! command -v pulseaudio &> /dev/null; then\n")
            sb.append("    echo \"正在安装 PulseAudio (Termux 原生音频服务)...\"\n")
            sb.append("    pkg install -y pulseaudio 2>/dev/null || {\n")
            sb.append("        echo \"警告: PulseAudio 安装失败，声音将不可用\"\n")
            ...
            sb.append("    sleep 0.5\n")
            sb.append("    # 杀掉 Termux 进程组内的所有 pulseaudio\n")
            sb.append("    rm -f \"\$HOME/.config/pulse/cookie\" \"\$HOME/.config/pulse/pid\" 2>/dev/null || true\n")
            // 启动 PA daemon（Termux 原生非 root 下稳定运行，不需要 allow-root/disable-shm）
            sb.append("    # 启动 PA daemon\n")
            sb.append("    PA_START_OK=0\n")
            sb.append("    pulseaudio --start --exit-idle-time=-1 --log-target=syslog 2>/dev/null\n")
            sb.append("    sleep 1\n")
            sb.append("    if pulseaudio --check 2>/dev/null || pactl info >/dev/null 2>&1; then\n")
            sb.append("        PA_START_OK=1\n")
            sb.append("        echo 'PA daemon 启动成功'\n")
            sb.append("    else\n")
            sb.append("        echo 'PA daemon 启动异常，尝试重启...'\n")
            sb.append("        pulseaudio --kill 2>/dev/null || true\n")
            sb.append("        sleep 0.5\n")
            sb.append("        pulseaudio -D --exit-idle-time=-1 --disallow-module-loading=false 2>/dev/null\n")
            sb.append("        sleep 1.5\n")
            sb.append("        if pulseaudio --check 2>/dev/null || pactl info >/dev/null 2>&1; then\n")
            sb.append("            PA_START_OK=1\n")
            sb.append("            echo 'PA daemon 重启成功'\n")
            sb.append("        fi\n")
            sb.append("    fi\n")
            // 加载模块（幂等，重复 load 安全忽略）
            sb.append("    if [ \"\$PA_START_OK\" = \"1\" ]; then\n")
            sb.append("        # 卸载默认加载的 ALSA sink（避免 QEMU 音频直接输出到物理扬声器\n")
            sb.append("        # 导致 auto_null.monitor 永远读不到数据）\n")
            sb.append("        pactl unload-module module-alsa-sink 2>/dev/null || true\n")
            sb.append("        pactl unload-module module-alsa-card 2>/dev/null || true\n")
            sb.append("        sleep 0.3\n")
            sb.append("        # 加载虚拟 null-sink：QEMU 输出目标，Android 播放器从它的 monitor 读 PCM\n")
            sb.append("        pactl load-module module-null-sink sink_name=auto_null sink_properties=device.description=Virtual-Sink >/dev/null 2>&1 || true\n")
            sb.append("        sleep 0.3\n")
            sb.append("        # 将 null-sink 设为默认 sink（即使 ALSA 模块已卸载，也显式指定防止竞态）\n")
            sb.append("        pactl set-default-sink auto_null 2>/dev/null || true\n")
            sb.append("        pactl set-default-source auto_null.monitor 2>/dev/null || true\n")
            sb.append("        sleep 0.3\n")
            sb.append("        # PA native protocol TCP（QEMU PA 客户端通过此端口连接）\n")
            sb.append("        pactl load-module module-native-protocol-tcp auth-ip-acl=\"127.0.0.1 ::1\" port=4713 >/dev/null 2>&1 || true\n")
            sb.append("        sleep 0.3\n")
            sb.append("        # PA simple protocol TCP（Android PulseAudioPlayer 从 auto_null.monitor 读 PCM 播放）\n")
            sb.append("        pactl load-module module-simple-protocol-tcp rate=44100 format=s16le channels=2 source=auto_null.monitor record=true port=4714 >/dev/null 2>&1 || true\n")
            ...
            sb.append("        done\n")
            sb.append("        if [ \"\$PA_READY\" = \"1\" ] && [ \"\$PA_SIMPLE_READY\" = \"1\" ]; then\n")
            sb.append("            # 诊断：确认当前默认 sink 确实是 auto_null\n")
            sb.append("            ACTUAL_DEFAULT_SINK=\$(pactl get-default-sink 2>/dev/null || echo unknown)\n")
            sb.append("            echo \"PA 就绪: default-sink=\$ACTUAL_DEFAULT_SINK (预期 auto_null)\"\n")
            sb.append("            echo 'PulseAudio TCP 4713 + 4714 双端口就绪（Termux 原生层）'\n")
            sb.append("            HAS_PULSEAUDIO=1\n")
            sb.append("        else\n")
            sb.append("            echo 'WARN: PulseAudio 端口未全部就绪，PA 音频不可用'\n")
            sb.append("            HAS_PULSEAUDIO=0\n")
            ...
            sb.append("    fi\n")
            sb.append("else\n")
            sb.append("    echo \"警告: pulseaudio 不可用，声音将不可用\"\n")
            sb.append("fi\n")
            ...
            sb.append("PA_SIMPLE_PROTO_PORT=4714\n")
            sb.append("ENV_EOF\n")
            sb.append("echo \"音频后端就绪 (mode=\$USER_AUDIO_MODE, vnc=\$HAS_VNC_AUDIO, pa=\$HAS_PULSEAUDIO)\"\n\n")
        } else {
            sb.append("echo \"跳过音频配置（声音已关闭）\"\n\n")
        }
        ...
        if (effectiveHasSound) {
            sb.append("# 音频策略判定（PA daemon 已在 Termux 原生层启动）\n")
            ...
            sb.append("fi\n")
            // 最终音频策略：
            //   VNC_RFB 模式且 HAS_VNC_AUDIO=1 => 走 VNC audio driver（音频随 VNC 走）
            //   其他情况（PA_FOLLOW_SCREEN / PA_PERSIST / VNC_RFB但driver不可用）=> 走原生层 PA
            ...
            sb.append("        echo '  [容器内] 音频策略: VNC RFB 扩展'\n")
            sb.append("    elif [ \"\${HAS_PULSEAUDIO:-0}\" = \"1\" ]; then\n")
            sb.append("        QEMU_USE_PULSEAUDIO=1\n")
            sb.append("        echo '  [容器内] VNC audio 不可用，回退到 Termux 原生层 PulseAudio'\n")
            sb.append("    else\n")
            sb.append("        echo '  [容器内] WARN: 无可用音频后端'\n")
            sb.append("    fi\n")
            sb.append("    ;;\n")
            sb.append("  ${AudioMode.PA_FOLLOW_SCREEN}|${AudioMode.PA_PERSIST})\n")
            sb.append("    if [ \"\${HAS_PULSEAUDIO:-0}\" = \"1\" ]; then\n")
            sb.append("        QEMU_USE_PULSEAUDIO=1\n")
            sb.append("        echo '  [容器内] 音频策略: Termux 原生层 PulseAudio'\n")
            sb.append("    else\n")
            sb.append("        echo '  [容器内] WARN: PulseAudio 未就绪，音频不可用'\n")
            sb.append("    fi\n")
            sb.append("    ;;\n")
            sb.append("  *)\n")
            sb.append("    echo '  [容器内] 音频: 跳过'\n")
            sb.append("    ;;\n")
            sb.append("esac\n")
            // 验证容器内能连上宿主 PA daemon（共享网络栈）
            sb.append("if [ \"\$QEMU_USE_PULSEAUDIO\" = \"1\" ]; then\n")
            sb.append("    if (echo > /dev/tcp/127.0.0.1/\${PA_TCP_PORT:-4713}) 2>/dev/null; then\n")
            sb.append("        echo \"  [容器内] ✓ 已连通 Termux 原生层 PulseAudio 127.0.0.1:\${PA_TCP_PORT:-4713}\"\n")
            sb.append("    else\n")
            sb.append("        echo \"  [容器内] WARN: 无法连通 Termux 原生层 PA (\${PA_TCP_PORT:-4713})，检查宿主 PA 是否运行\"\n")
            sb.append("        QEMU_USE_PULSEAUDIO=0\n")
            sb.append("    fi\n")
            sb.append("fi\n\n")
        }
        ...
        if (effectiveHasSound) {
            ...
            sb.append("    # server=tcp:127.0.0.1:4713 让 QEMU 的 PA 客户端连 Termux 原生层 PA daemon\n")
            sb.append("    # sink=auto_null 确保输出到我们的 null-sink（monitor 供 Android 播放器读取）\n")
            sb.append("    EXTRA_AUDIO_ARGS=(\n")
            sb.append("        -audiodev pa,id=pa_audio,server=tcp:127.0.0.1:4713,sink=auto_null\n")
            sb.append("        -device ich9-intel-hda\n")
            sb.append("        -device hda-output,audiodev=pa_audio\n")
        ...
        // ========== 注：PA daemon 已在 Termux 原生层启动 ==========
        // 不再需要容器内预配置 PA（machine-id / client.conf / daemon.conf / default.pa /
        // allow-root / disable-shm 等 workaround），proot 共享网络栈，QEMU 直接连宿主 127.0.0.1:4713。
