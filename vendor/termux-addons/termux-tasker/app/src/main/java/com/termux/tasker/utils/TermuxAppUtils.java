...
public class TermuxAppUtils {
    ...
    public static String checkIfTermuxAppIsInstalledAndAccessible(final Context context) {
        ...
        try {
            // When the plugin is integrated as a library into the host Termux app, the host
            // package name is the context package (e.g. com.termux) and will always exist.
            // Fall back to the legacy Constants.TERMUX_PACKAGE ("com.termux") when running
            // as a standalone APK so the shared code path still works.
            String termuxPackageName = context.getPackageName();
            try {
                applicationInfo = packageManager.getApplicationInfo(termuxPackageName, 0);
